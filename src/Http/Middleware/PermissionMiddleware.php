...
{
    public function handle(Request $request, Closure $next, $permission, $resourceType = null)
    {
        $user = Auth::user();
        ...
        $hasPermission = $modelRole->role->permissions()->whereIn('name', $permissions)->exists();
        if (!$hasPermission) {
            return response()->json(['message' => 'Insufficient permissions'], 403);
        }
